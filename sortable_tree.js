...
SortableTree.Node = Class.create({
    initialize: function(tree, parent, element, options) {
        ...
        this.droppable_options = Object.extend({
            onHover: function(drag, drop, overlap) {
                this.onHover(drag, drop, overlap);
            }.bind(this),
            onDrop: function(drag, drop, event) {
                this.onDrop(drag, drop, event);
            }.bind(this),
            overlap: 'vertical',
    ...
    onDrop: function(drag, drop, event) {
        ...
        if (drop.parent || this.dropPosition == 'insert') {
            ...
            case 'top':
                drop.parent.insertBefore(drag, drop);
                break;
            case 'bottom':
                drop.parent.insertBefore(drag, drop.nextSibling());
                break;
            case 'insert':
    ...
    to_params: function(name) {
        ...
        return name + '[' + this.id() + '][parent_id]=' + this.parent.id() + '&' + name + '[' + this.id() + '][left_id]=' + (leftNode ? leftNode.id() : '');
        // null
    },
